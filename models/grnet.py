...
sys.path.append(os.path.join(proj_dir, "utils/Pointnet2_PyTorch/pointnet2"))
from utils.mm3d_pn2.ops.furthest_point_sample.furthest_point_sample_utils import furthest_point_sample
from utils.mm3d_pn2.ops.group_points.group_points import grouping_operation
from utils.mm3d_pn2.ops.gather_points.gather_points import gather_points

from utils.gridding import Gridding, GriddingReverse
from utils.cubic_feature_sampling import CubicFeatureSampling
from utils.grnet_model_utils import calc_cd, calc_emd
...
class RandomPointSampling(torch.nn.Module):
    ...
    def forward(self, pred_cloud, partial_cloud=None):
        if partial_cloud is not None:
            partial_cloud = partial_cloud.transpose(1, 2)
...
class Model(torch.nn.Module):
    ...
    def forward(self, partial_cloud, gt, is_training=True, mean_feature=None, alpha=None):
        ...
        if self.num_points < 16384:
            idx_fps = furthest_point_sample(dense_cloud, self.num_points)
            dense_cloud = gather_points(dense_cloud.transpose(1, 2).contiguous(), idx_fps).transpose(1, 2)
        ...
        if is_training:
            if self.train_loss == 'emd':
                ...
                loss1, _ = calc_cd(sparse_cloud, gt)  # cd_t
                loss2, _ = calc_cd(dense_cloud, gt)  # cd_t
            else:
