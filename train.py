...
import sys
import argparse
from pcn_data_loader import PCNDataset
            ...
            if i % args.step_interval_to_print == 0:
                logging.info(exp_name + ' train [%d: %d/%d]  loss_type: %s, loss2: %f total_loss: %f lr: %f' %
                             (epoch, i, len(dataset) / args.batch_size, args.loss,
                              loss2.mean().item(), net_loss.mean().item(), lr) + ' alpha: ' + str(alpha))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', help='path to config file', default='cfgs/msn.yaml')
